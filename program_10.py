...
def CalcTqmean(Qvalues):
    ...
    # calculate the mean streamflow for the serie of data
    mean_Q = Qvalues.mean()
...
def CalcRBindex(Qvalues):
    ...
    # calculate the sum of the absolute values of day-to-day changes in daily discharge
    sum_abs = 0.0    
...
def Calc7Q(Qvalues):
    ...
    # obtain the lowest average flow in any 7-day period during the year
...
def CalcExceed3TimesMedian(Qvalues):
    ...
    # find the index of the daily streamflow greater than 3 times the median flow
    index=( Qvalues > 3*Qvalues.median() )

    # calculate the number of days with flows greater than the median flow
    median3x = index.sum()
...
def GetAnnualStatistics(DataDF):
    ...
    # obtain the water years range of the data
    # according to USGS, the water year is designated by the calendar year in which it ends and which includes 9 of the 12 months
    start_Year = 1970
    end_Year = 2020
    WYrange = range(start_Year,end_Year)
    
    # define a new dataframe to store annual (water year) metric values, set the index as the "water year"
...
def GetMonthlyStatistics(DataDF):
    ...
    # calculate the metrics for each month in each water year
...
def GetAnnualAverages(WYDataDF):
    ...
    # calculate the mean value for each matric
